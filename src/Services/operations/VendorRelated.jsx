...
const { GET_FILTERED_VENDORS, GET_FILTERED_VENDORS_WITH_MINIMUM_DETAILS } =
  getVendorEndpoints;

export function getfilteredVendorsList(
  ...
  return async () => {
    if (socket) {
      ...
      try {
        ...
        toast.error(
          error?.response?.data?.message ||
            'Unable to fetch vendor data. Please try again.'
        );
      }
    }
  };
  ...
  return async (dispatch) => {
    if (socket) {
      ...
      try {
        const response = await apiConnector(
          'POST',
          getOrdersEndpoints.GET_VENDOR_ORDERS,
          { vendorID },
          { Authorization: `Bearer ${token}` }
        );
        ...
        toast.error(
          error?.response?.data?.message ||
            'Unable to fetch ongoing orders. Please try again.'
        );
      }
    }
  };
