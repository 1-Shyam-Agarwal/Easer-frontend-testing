...
export async function resendOTP(dispatch , setSecondaryDisable , email , setTimer , type)
{
...
export async function handleVerifyOtpAndCreateAccount(otpValue, 
                                                      data ,
...
export async function handleSaveGoogleSignupAllDetails(information , setGoogleSignupAdditionalDetailsDialogBox ,navigate , dispatch)
{
        const toastId = toast.loading("Loading...");
        try
        {
...
export async function handlePreCustomLoginChecksAndSendOtp(email , setDisable , setData , setWindowSwitch , dispatch)
{
        dispatch(setDisable(true));
        const toastId = toast.loading("Loading");
        ...
        try
        {
            ...
            let data ; 
            if(response?.data?.data?.role) //This indicates that user is admin or vendor 
            {
                data = {...response?.data?.data , "email" : email};
            }
            else // This means user is customer
            {
...
export async function handleVerifyLoginOtp( email , otp , setVerifyOTPDisabled ,dispatch , navigate)
{
    //Disabling the verifyOTP button to avoid customer to make multiple calls
    setVerifyOTPDisabled(true);
    const toastId = toast.loading("Verfying...");
...
export async function verifyPasswordHandler(email , password , navigate, setDisable, dispatch)
{
      dispatch(setDisable(true));
      const toastId = toast.loading("Loading...");
      try {
