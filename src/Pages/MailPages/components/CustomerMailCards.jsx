  ...
  return (
    <div className="flex  flex-col divide-y divide-gray-200 bg-white">
      {mails?.map((mail, index) => (
        <div
          key={index}
          className="group  relative flex flex-col sm:flex-row sm:items-center  justify-between gap-2 px-4 sm:px-6 py-2 sm:py-4 transition-all duration-200 hover:bg-gray-50 hover:shadow-sm rounded-md cursor-pointer"
          onClick={() => navigate(`mail/${mail.mail_id}`)}
        >

          {/* Receiver */}
          <div className="flex items-center gap-2 w-full sm:w-1/4 min-w-[150px]">
            <div className="bg-blue-100 text-blue-600 font-semibold rounded-full px-1 py-[1px] sm:px-2 sm:py-1 text-[10px] sm:text-xs">
              To
            </div>
            <div className="flex flex-col truncate capitalize">
              <span className="text-[0.65rem] sm:text-sm font-normal text-gray-700 truncate">
                {mail.receiver?.vendorAdditionalDetails?.shopName}
              </span>
              <span className="text-[0.55rem] sm:text-xs text-gray-500 truncate">
          ...
          <div className="w-full sm:w-1/3 overflow-hidden">
            <div className="flex flex-wrap gap-1 sm:gap-2">
              {mail.documents?.slice(0, 2).map((doc, i) => (
                <span
                  key={i}
                  className="bg-green-100 text-green-600 text-[0.5rem] sm:text-xs font-medium px-2 py-[3px] sm:py-1 rounded-full max-w-[110px] truncate"
                  title={doc.fileName}
                >
          ...
          {/* Time */}
          <div className="text-right w-full absolute top-0 right-0 py-2 px-3 sm:relative sm:w-1/6 text-[8px] sm:text-xs text-gray-500 group-hover:text-gray-700">
